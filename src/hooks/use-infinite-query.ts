...
import { useEffect, useRef, useSyncExternalStore, useCallback, useState } from 'react';
...
>(props: UseInfiniteQueryProps<T>) {
  ...
  // Create a stable subscribe function that always uses the current store
  const subscribe = useCallback(
    (listener: () => void) => {
      return storeRef.current.subscribe(listener);
    },
    [storeVersion],
  ); // Re-create when store version changes
  ...
  useEffect(() => {
    ...
    // Check if any of the props that affect the query have changed
    const propsChanged =
      currentProps.tableName !== previousProps.tableName ||
      currentProps.columns !== previousProps.columns ||
      currentProps.pageSize !== previousProps.pageSize ||
      currentProps.trailingQuery !== previousProps.trailingQuery;

    // Recreate store if props have changed and we've had an initial fetch
    if (storeRef.current.getState().hasInitialFetch && propsChanged) {
      storeRef.current = createStore<TData, T>(currentProps);
      // Increment store version to trigger re-subscription
      setStoreVersion((prev) => prev + 1);
      // Immediately initialize the new store to fetch data with new filters
      if (typeof window !== 'undefined') {
        storeRef.current.initialize();
      }
    }

    // Initialize if we haven't fetched yet (for initial load)
