...
export class GitHubAppService extends AgentsmithSupabaseService {
  ...
  async verifyInstallation(options: VerifyInstallationOptions) {
    ...
    try {
      ...
      this.logger.error(error, 'Error fetching installation repositories');
    ...
    if (error) {
      this.logger.error(error, 'Error fetching installation');
    ...
    if (isValidOrganization) {
      ...
      if (error) {
        this.logger.error(error, 'Failed to update installation status');
  ...
  async createInstallationRepositories(options: CreateInstallationRepositoriesOptions) {
    ...
    if (error) {
      this.logger.error(error, 'Failed to create project repositories');
      throw error;
    }
  }
  ...
  async getProjectRepository(projectId: number) {
    ...
    if (error) {
      this.logger.error(error, 'Failed to fetch project repository');
      throw error;
    }
