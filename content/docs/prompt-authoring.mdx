...
<video
  ...
  poster={LANDING_VIDEOS.robustAuthoring.cover.src}
  controls
>
  <source src={LANDING_VIDEOS.robustAuthoring.webm} type="video/webm" />
  <source src={LANDING_VIDEOS.robustAuthoring.mp4} type="video/mp4" />
...
<Alert variant="warning">
  ...
  <AlertDescription>
    ...
    after it is synced. The SDK will log a "prompt version not found" error, but will continue with
    the execution.
  </AlertDescription>
</Alert>
...
<video
  ...
  playsInline
  className="w-full rounded-sm border border-muted"
  poster={LANDING_VIDEOS.unifiedApi.cover.src}
  controls
>
  <source src={LANDING_VIDEOS.unifiedApi.webm} type="video/webm" />
  <source src={LANDING_VIDEOS.unifiedApi.mp4} type="video/mp4" />
  Your browser does not support the video tag.
</video>
