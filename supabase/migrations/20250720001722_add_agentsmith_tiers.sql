...
create table if not exists agentsmith_tiers (
    ...
    name text not null,
    stripe_product_id text null references stripe.products(id),
...
alter table organizations
    ...
    add column if not exists stripe_customer_id text references stripe.customers(id);
...
values
  ...
  ('HOBBY', 'Hobby', 100, 3, 3, 30000, 30, true),
  ('PRO', 'Pro', 9999, 9999, 10, 100000, 90, true),
  ('ENTERPRISE', 'Enterprise', 9999, 9999, 100, 1000000, 365, true);
...
as $$
    with user_limit as (
        select coalesce(t.user_limit, 1) as limit
        from public.organizations o
        left join public.agentsmith_tiers t on t.id = o.agentsmith_tier_id
    ...
    select user_count.count < user_limit.limit
