alter table llm_logs add column prompt_tokens integer;
alter table llm_logs add column completion_tokens integer;
...
alter table llm_logs add column duration_ms integer;
alter table llm_logs add column cost_usd numeric;
alter table llm_logs add column model text;
alter table llm_logs add column provider text;
alter table llm_logs add column tps numeric;
...
begin
  ...
  if new.raw_output is not null and (old.raw_output is null or old.raw_output != new.raw_output) then
    ...
    if new.duration_ms is not null and new.duration_ms > 0 then
      new.tps := new.total_tokens / (new.duration_ms / 1000);
    end if;
  end if;
...
-- Create trigger to automatically update the new columns when raw_output is updated
create trigger trigger_update_llm_log_metrics
  before update on llm_logs
  for each row
  execute function update_llm_log_metrics();
