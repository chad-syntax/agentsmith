...
create type agentsmith_event_severity as enum (
    'DEBUG',
    'INFO',
    'WARN',
    'ERROR'
...
create type agentsmith_event_type as enum (
    'SYNC_START',
    'SYNC_COMPLETE',
    'SYNC_ERROR',
    'ALERT'
...
create table global_contexts (
    id bigint generated by default as identity not null primary key,
    project_id bigint references projects(id) not null unique,
...
-- create a global context record when a project is created
create or replace function create_project_global_context()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
    ...
    insert into public.global_contexts (project_id, content)
    values (var_project_id, '{}');

    return new;
end;
$$;

create trigger trigger_create_project_global_context
    after insert on projects
    for each row
    execute function create_project_global_context();
...
create index on prompt_variables(uuid);
create index on prompt_variables(created_at);
create index on prompt_variables(updated_at);
...
create table project_repositories (
    ...
    pr_workflow boolean not null default true,
...
create table agentsmith_events (
    id bigint generated by default as identity not null primary key,
    uuid uuid not null default uuid_generate_v4(),
    type agentsmith_event_type not null default 'ALERT',
    name text not null,
    description text not null,
    severity agentsmith_event_severity not null default 'INFO',
    organization_id bigint references organizations(id) not null,
    project_id bigint references projects(id),
    details jsonb not null default '{}',
    created_by bigint references agentsmith_users(id) default agentsmith_user_id(),
    created_at timestamptz not null default now(),
    updated_at timestamptz not null default now()
);

create index on agentsmith_events(organization_id);
create index on agentsmith_events(project_id);
create index on agentsmith_events(created_at);
create index on agentsmith_events(updated_at);
create index on agentsmith_events(uuid);

alter table agentsmith_events enable row level security;

create policy "Users can view events for organizations they belong to"
    on agentsmith_events for select
    to authenticated
    using (is_organization_member(organization_id));

create policy "Users can create events for organizations they belong to"
    on agentsmith_events for insert
    to authenticated
    with check (is_organization_member(organization_id));

create trigger trigger_updated_at_agentsmith_events
...
grant select on table public.global_contexts to github_webhook;
...
create policy "Allow github_webhook select access on global_contexts"
