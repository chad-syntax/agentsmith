...
var require_verifier = __commonJS({
  "../node_modules/fast-jwt/src/verifier.js"(exports2, module2) {
    ...
    function validateClaimValues(values, claim, allowed, arrayValue) {
      const failureMessage = arrayValue ? `Not all of the ${claim} claim values are allowed.` : `The ${claim} claim value is not allowed.`;
...
var require_signer = __commonJS({
  "../node_modules/fast-jwt/src/signer.js"(exports2, module2) {
    ...
    module2.exports = function createSigner2(options) {
      let {
        key,
        algorithm,
        noTimestamp,
...
var require_modifiers = __commonJS({
  "../node_modules/fast-redact/lib/modifiers.js"(exports2, module2) {
        ...
        if (wc) {
          ...
          for (var j = 0; j < wcKeys.length; j++) {
            ...
            if (consecutive) {
              ...
              level2 = i;
              ov = iterateNthLevel(wcov, level2 - 1, k, path2, afterPath, censor, isCensorFct, censorFctTakesPath, originalKey, n, nv, ov, kIsWc, wck, i, lastPathIndex, redactPathCurrent, store, o[originalKey], depth + 1);
            } else {
    ...
    function iterateNthLevel(wcov, level2, k, path2, afterPath, censor, isCensorFct, censorFctTakesPath, originalKey, n, nv, ov, kIsWc, wck, i, lastPathIndex, redactPathCurrent, store, parent, depth) {
      if (level2 === 0) {
        if (kIsWc || typeof wcov === "object" && wcov !== null && k in wcov) {
          if (kIsWc) {
            ov = wcov;
      ...
      for (const key in wcov) {
        if (typeof wcov[key] === "object") {
          ...
          iterateNthLevel(wcov[key], level2 - 1, k, path2, afterPath, censor, isCensorFct, censorFctTakesPath, originalKey, n, nv, ov, kIsWc, wck, i, lastPathIndex, redactPathCurrent, store, parent, depth + 1);
        }
      }
    }
    function tree() {
...
__export(index_exports, {
  AgentsmithClient: () => AgentsmithClient,
  Prompt: () => Prompt
});
module.exports = __toCommonJS(index_exports);

// src/AgentsmithClient.ts
...
var import_fast_jwt = __toESM(require_src());

// ../src/lib/logger.ts
var import_pino = __toESM(require_pino());
var level = process.env.VERCEL_ENV === "production" ? "info" : process.env.VERCEL_ENV === "preview" ? "debug" : "debug";
...
var createJwtClient = (jwt, supabaseUrl, supabaseAnonKey) => {
  if (!jwt) {
    throw new Error("JWT is required");
  }
  return (0, import_supabase_js.createClient)(
    supabaseUrl ?? "https://ehpaperavkeicrwhvsbx.supabase.co",
    supabaseAnonKey ?? "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVocGFwZXJhdmtlaWNyd2h2c2J4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDc3NTA3NTQsImV4cCI6MjA2MzMyNjc1NH0.rntsQRcyVBb9UTahY4LSq5o4vuUSYfZS6h4NUK8qme0",
    {
      global: {
        headers: {
...
// ../src/lib/sync/repo-paths.ts
var promptJsonFilePath = (options) => `${options.agentsmithFolder}/prompts/${options.promptSlug}/prompt.json`;
var versionJsonFilePath = (options) => `${options.agentsmithFolder}/prompts/${options.promptSlug}/${options.version}/version.json`;
var variablesJsonFilePath = (options) => `${options.agentsmithFolder}/prompts/${options.promptSlug}/${options.version}/variables.json`;
var contentJ2FilePath = (options) => `${options.agentsmithFolder}/prompts/${options.promptSlug}/${options.version}/content.j2`;
var globalsJsonFilePath = (options) => `${options.agentsmithFolder}/globals.json`;
...
var routes = {
  marketing: {
    ...
    roadmap: (proposeModal) => `/roadmap${proposeModal ? "?proposeModal=true" : ""}`,
    roadmapItem: (itemSlug) => `/roadmap/${itemSlug}`
  },
  studio: {
    home: "/studio",
    organization: (organizationUuid) => `/studio/organization/${organizationUuid}`,
    settings: (organizationUuid) => `/studio/organization/${organizationUuid}/settings`,
    editOrganization: (organizationUuid) => `/studio/organization/${organizationUuid}/edit`,
    project: (projectUuid) => `/studio/project/${projectUuid}`,
    editProject: (projectUuid) => `/studio/project/${projectUuid}/edit`,
    prompts: (projectUuid) => `/studio/project/${projectUuid}/prompts`,
    promptDetail: (projectUuid, promptId) => `/studio/project/${projectUuid}/prompts/${promptId}`,
    editPromptVersion: (projectUuid, promptVersionUuid) => `/studio/project/${projectUuid}/prompts/edit/${promptVersionUuid}`,
    logs: (projectUuid) => `/studio/project/${projectUuid}/logs`,
    logDetail: (projectUuid, logId) => `/studio/project/${projectUuid}/logs/${logId}`,
    account: `/studio/account`,
    resetPassword: `/studio/reset-password`,
    joinOrganization: (organizationInviteCode) => `/join/${organizationInviteCode}`,
    events: (projectUuid) => `/studio/project/${projectUuid}/events`,
    eventDetail: (projectUuid, eventUuid) => `/studio/project/${projectUuid}/events/${eventUuid}`,
    projectGlobals: (projectUuid) => `/studio/project/${projectUuid}/globals`,
  ...
  api: {
    v1: {
      executePromptVersion: (promptVersionUuid) => `/api/v1/promptVersion/${promptVersionUuid}/execute`,
      compilePromptVersion: (promptVersionUuid) => `/api/v1/promptVersion/${promptVersionUuid}/compile`,
      sdkExchange: "/api/v1/sdk-exchange"
    }
  },
  github: {
    createInstallation: (appName, state) => `https://github.com/apps/${appName}/installations/new?state=${state}`,
    installation: (installationId) => `https://github.com/settings/installations/${installationId}`,
    repository: (repositoryFullName) => `https://github.com/${repositoryFullName}`,
    branch: (repositoryFullName, branchName) => `https://github.com/${repositoryFullName}/tree/${branchName}`
  },
  openrouter: {
  ...
  error: (message) => `/error?message=${encodeURIComponent(message)}`,
  external: {
    github: "https://github.com/chad-syntax/agentsmith",
    stripe: {
      checkout: {
        proAlphaClub: process.env.VERCEL_ENV === "production" ? "https://buy.stripe.com/dRmfZi8M39K68y92566wE01?prefilled_promo_code=ALPHACLUB" : "https://buy.stripe.com/test_dRm7sN7Z39LTcII4EO53O01?prefilled_promo_code=ALPHACLUB"
      }
    }
  },
  emails: {
...
var import_nunjucks = __toESM(require("nunjucks/browser/nunjucks"));
var import_transformer = require("nunjucks/src/transformer");
var import_lodash = __toESM(require("lodash.merge"));
var createParsedVariable = (name, type) => ({
  name,
  type
});
var TemplateVariablesExtractor = class {
  constructor() {
    this.variableMap = /* @__PURE__ */ new Map();
    this.localVariables = /* @__PURE__ */ new Set();
  }
  /* ---------- Utilities ---------- */
  ensureVariablePath(path2, finalType = "STRING") {
    if (path2.length === 0) return null;
    let current = this.variableMap.get(path2[0]);
    if (!current) {
      current = createParsedVariable(path2[0], path2.length === 1 ? finalType : "JSON");
      this.variableMap.set(path2[0], current);
    }
  ...
  ensurePathOnVariable(root, path2, finalType = "STRING") {
    let current = root;
  ...
  traverseNode(node, ctx = {}) {
    ...
    switch (node.typename) {
      ...
      case "LookupVal": {
        const path2 = this.literalPathFromLookup(node);
        if (!path2) {
          this.traverseNode(node.val, ctx);
          this.traverseNode(node.target, ctx);
          break;
        }
        if (ctx.currentCollection && ctx.loopItemName && path2[0] === ctx.loopItemName && path2.length > 1) {
          this.ensurePathOnVariable(ctx.currentCollection, path2.slice(1));
        } else if (!(ctx.loopItemName && path2.length === 1 && path2[0] === ctx.loopItemName)) {
          if (!this.localVariables.has(path2[0])) {
            this.ensureVariablePath(path2);
          }
        }
        break;
      }
      case "For": {
        ...
        if (collectionPath && !this.localVariables.has(collectionPath[0])) {
          collectionVar = this.ensureVariablePath(collectionPath, "JSON") ?? void 0;
...
var extractTemplateVariables = (content) => {
  try {
    const ast = (0, import_transformer.transform)(import_nunjucks.default.parser.parse(content));
...
var DISALLOWED_PROPERTY_LOOKUPS = ["__proto__", "constructor", "prototype"];
var ensureSecureAstRecursive = (node) => {
  if (!node) return;
  if (Array.isArray(node)) {
  ...
  switch (node.typename) {
    case "Symbol":
      ...
      break;
    case "LookupVal":
      if (node.val && node.val.typename === "Literal" && typeof node.val.value === "string" && DISALLOWED_PROPERTY_LOOKUPS.includes(node.val.value)) {
    ...
    default:
      if (typeof node === "object") {
        for (const key in node) {
          if (Object.prototype.hasOwnProperty.call(node, key) && !key.startsWith("_") && key !== "parent") {
...
var ensureSecureAst = (content) => {
  try {
    const ast = import_nunjucks.default.parser.parse(content);
...
var findMissingGlobalContext = (options) => {
  const { globalVariable, globalContext } = options;
  const missingGlobalContext = [];
...
var processUniqueValues = (variables) => {
...
var validateGlobalContext = (content, globalContext) => {
...
var compilePrompt = (promptContent, variables) => {
  ensureSecureAst(promptContent);
  const processedVariables = processUniqueValues(variables);
  import_nunjucks.default.configure({ autoescape: false });
  return import_nunjucks.default.renderString(promptContent, processedVariables);
};
var validateVariables = (variables, variablesToCheck) => {
  const missingRequiredVariables = variables.filter((v) => v.required).filter((v) => !(v.name in variablesToCheck));
  const defaultValues = variables.reduce(
    (acc, v) => v.default_value ? { ...acc, [v.name]: v.default_value } : acc,
    {}
  );
  const variablesWithDefaults = (0, import_lodash.default)(defaultValues, variablesToCheck);
  return { missingRequiredVariables, variablesWithDefaults };
};

// ../src/utils/versioning.ts
var compareSemanticVersions = (version1, version2) => {
...
// ../src/lib/AgentsmithService.ts
var AgentsmithService = class {
  constructor(options) {
...
// ../src/lib/AgentsmithSupabaseService.ts
var AgentsmithSupabaseService = class extends AgentsmithService {
  constructor(options) {
    super(options);
...
// ../src/utils/gen-alpha-numeric.ts
var genAlphaNumeric = (length) => {
...
var STUDIO_FULL_HEIGHT = `h-[calc(100vh-${HEADER_HEIGHT}px)]`;
var IS_WAITLIST_REDIRECT_ENABLED = process.env.VERCEL_TARGET_ENV === "production" || process.env.VERCEL_TARGET_ENV === "staging";
var VALID_ROADMAP_ITEM_STATES = [
  "PROPOSED",
  "REJECTED",
  "PLANNED",
  "IN_PROGRESS",
...
var VALID_STATUSES = [...VALID_ROADMAP_ITEM_STATES];

// ../src/lib/OrganizationsService.ts
var OrganizationsService = class extends AgentsmithSupabaseService {
  constructor(options) {
    super({ ...options, serviceName: "organizations" });
  }
  async getOrganizationId(organizationUuid) {
    const { data, error } = await this.supabase.from("organizations").select("id").eq("uuid", organizationUuid).single();
    if (error) {
  ...
  async getOrganizationData(organizationUuid) {
    const { data, error } = await this.supabase.from("organizations").select("*, projects(*), organization_users(id, role, agentsmith_users(*))").eq("uuid", organizationUuid).single();
    if (error) {
  ...
  async getProjectRepositoriesForOrganization(organizationId) {
    const { data, error } = await this.supabase.from("project_repositories").select(
      "*, github_app_installations!inner(status), organizations!inner(id), projects(uuid, name)"
    ).eq("github_app_installations.status", "ACTIVE").eq("organizations.id", organizationId);
    if (error) {
  ...
  async getOrganizationKeySecret(organizationUuid, key) {
  ...
  async getOrCreateOpenrouterCodeVerifier(organizationUuid) {
...
// ../src/lib/LLMLogsService.ts
var LLMLogsService = class extends AgentsmithSupabaseService {
  constructor(options) {
    super({ ...options, serviceName: "llmLogs" });
  }
  async createLogEntry(options) {
    const { projectId, promptVersionId, variables, rawInput } = options;
    const { data, error } = await this.supabase.from("llm_logs").insert({
  ...
  async updateLogWithCompletion(uuid, rawOutput) {
    const { data, error } = await this.supabase.from("llm_logs").update({
  ...
  async getLogsByProjectId(projectId) {
    const { data, error } = await this.supabase.from("llm_logs").select("*, projects(*), prompt_versions(*, prompts(*))").eq("project_id", projectId).order("created_at", { ascending: false });
    if (error) {
  ...
  async getLogByUuid(uuid) {
    const { data, error } = await this.supabase.from("llm_logs").select(
      `
        *,
        projects(*),
...
var VaultService = class extends AgentsmithSupabaseService {
  constructor(options) {
   ...
   */
  async createOrganizationKey(options) {
    try {
   ...
   */
  async deleteOrganizationKey(organizationUuid, key) {
    try {
   ...
   */
  async replaceOrganizationKey(options) {
    try {
      const { organizationUuid, key, value, description } = options;
  ...
  async getOrganizationKeySecret(organizationUuid, key) {
    try {
      const { data, error } = await this.supabase.from("organizations").select("organization_keys!inner(key, vault_secret_id)").eq("uuid", organizationUuid).eq("organization_keys.key", key).single();
      if (error) {
...
var Prompt = class {
  ...
  async init() {
    try {
      ...
      const promptJson = await import_fs.default.promises.readFile(promptJsonPath, "utf-8");
      ...
      const targetVersion = this.argVersion === "latest" && prompt.latestVersion !== null ? prompt.latestVersion : this.argVersion;
      const versionJsonPath = versionJsonFilePath({
        agentsmithFolder: this.client.agentsmithDirectory,
        promptSlug: this.slug,
        version: targetVersion
      });
      const versionJsonFileContent = await import_fs.default.promises.readFile(versionJsonPath, "utf-8");
      const versionJson = JSON.parse(versionJsonFileContent);
      const contentJ2Path = contentJ2FilePath({
        agentsmithFolder: this.client.agentsmithDirectory,
        promptSlug: this.slug,
        version: targetVersion
      });
      const contentJ2FileContent = await import_fs.default.promises.readFile(contentJ2Path, "utf-8");
      this.version = {
      ...
      try {
        const variablesJsonFileContent = await import_fs.default.promises.readFile(variablesJsonPath, "utf-8");
        const variablesJson = JSON.parse(variablesJsonFileContent);
    ...
    if (fetchSpecificVersion) {
      const { data: data2, error: error2 } = await this.client.supabase.from("prompt_versions").select(
        "uuid, version, config, content, prompt_variables(uuid, name, type, required, default_value), prompts!inner(uuid, name, slug)"
      ).eq("prompts.slug", this.slug).eq("version", this.argVersion).single();
      if (error2) {
        console.error(`Failed to fetch ${this.slug}@${this.argVersion} from remote`, error2);
        throw error2;
      }
      const { prompts: prompts2, prompt_variables: prompt_variables2, ...version2 } = data2;
      this.meta = prompts2;
      this.version = {
        ...version2,
        config: version2.config
      };
      this.variables = prompt_variables2;
      return;
    }
    const { data, error } = await this.client.supabase.from("prompt_versions").select(
      "uuid, version, config, content, prompt_variables(uuid, name, type, required, default_value), prompts!inner(uuid, name, slug)"
    ).eq("prompts.slug", this.slug).eq("status", "PUBLISHED");
    if (error) {
  ...
  async compile(...args) {
    const variables = args[0] !== void 0 ? args[0] : {};
    const optionsArg = args.length === 2 ? args[1] : args.length === 1 && typeof args[0] === "object" && args[0] !== null && "globals" in args[0] ? args[0] : {};
    const { globals: globalOverrides } = optionsArg ?? { globals: {} };
    const globals = await this.client.initializeGlobals();
    ...
    const finalGlobalContext = (0, import_lodash2.default)(globals, globalOverrides);
  ...
  async execute(...args) {
    ...
    const optionsArg = args.length === 2 ? args[1] : args.length === 1 && typeof args[0] === "object" && args[0] !== null && ("globals" in args[0] || "config" in args[0]) ? args[0] : {};
    const { config: configOverrides } = optionsArg ?? {
      config: {}
    };
    const finalConfig = (0, import_lodash2.default)(this.version.config, configOverrides, {
      usage: {
        include: true
      }
    });
    ...
    const { log_uuid, organization_uuid } = data;
    try {
      const { value: openrouterApiKey, error: error2 } = await organizationsService.getOrganizationKeySecret(
        organization_uuid,
        ORGANIZATION_KEYS.OPENROUTER_API_KEY
      );
      if (error2) {
        throw new Error(`Failed to get OpenRouter API key: ${error2}`);
      }
      if (!openrouterApiKey) {
...
// src/AgentsmithClient.ts
var defaultAgentsmithDirectory = import_path.default.join(process.cwd(), "agentsmith");
var AgentsmithClient = class {
  constructor(sdkApiKey, projectId, options = {}) {
    this.fetchGlobalsPromise = null;
    this.projectGlobals = null;
    this.sdkApiKey = sdkApiKey;
    this.projectUuid = projectId;
    this.agentsmithApiRoot = options.agentsmithApiRoot ?? "https://staging.agentsmith.app";
    this.supabaseUrl = options.supabaseUrl ?? "https://ehpaperavkeicrwhvsbx.supabase.co";
    this.supabaseAnonKey = options.supabaseAnonKey ?? "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVocGFwZXJhdmtlaWNyd2h2c2J4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDc3NTA3NTQsImV4cCI6MjA2MzMyNjc1NH0.rntsQRcyVBb9UTahY4LSq5o4vuUSYfZS6h4NUK8qme0";
  ...
  async exchangeApiKeyForJwt(sdkApiKey) {
    try {
  ...
  async fetchGlobals() {
    try {
      ...
      const globalsJson = await import_fs2.default.promises.readFile(globalsJsonPath, "utf-8");
      this.projectGlobals = JSON.parse(globalsJson);
      return this.projectGlobals;
    } catch (err) {
      console.error("Failed to fetch globals from the file system", err);
    }
    try {
      const { data, error } = await this.supabase.from("global_contexts").select("*, projects(uuid)").eq("projects.uuid", this.projectUuid).single();
      if (error) {
        this.fetchGlobalsPromise = null;
        throw error;
      }
      ...
      this.projectGlobals = data.content;
      return this.projectGlobals;
    } catch (err) {
      this.fetchGlobalsPromise = null;
      throw err;
